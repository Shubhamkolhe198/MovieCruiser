...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers("/customer-ang").hasRole("user")
				.antMatchers("/favorite-ang").hasRole("user").antMatchers("/add-ang/**").hasRole("user")
				.antMatchers("/delete-ang/**").hasRole("user");
		http.formLogin();
	 ...
	 * .withUser("user") .password(("{noop}userpwd")) .roles("user") .and()
